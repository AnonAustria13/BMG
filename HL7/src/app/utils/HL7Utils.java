...
public class HL7Utils
{
    ...
    public static Object toObject(String data, Class<?> targetClass)
    {
        ...
        if (targetClass == Double.class)
            // replaceAll leaves only valid double format (max. one '-' in the front, one '.' in the middle)
            return Double.parseDouble("0" + data.replaceAll(
                ...
                "^[^\\d\\-]*?(?=(\\-?\\d+))|"+
                // Any non-decimals except '.' following a decimal (excluding first characters from above line)
                "(?<=\\d)[^\\d\\.]+|"+
                // Any dot followed by another dot that is - in turn - followed by a decimal
                "\\.(?=([\\s\\S]*\\.(?=([\\s\\S]*?\\d))))|"+
                // Any dot not followed by a decimal (removes last dot in case it would stand at the end of the number)
                "\\.(?=\\D*$)"
                , ""));

        if (targetClass == Float.class)
            // replaceAll leaves only valid float format (max. one '-' in the front, one '.' in the middle)
            return Double.parseDouble("0" + data.replaceAll(
